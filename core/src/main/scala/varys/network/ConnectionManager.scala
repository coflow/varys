...
private[varys] case class ConnectionManagerId(host: String, port: Int) {
  def toSocketAddress() = new InetSocketAddress(host, port)
}

private[varys] object ConnectionManagerId {
  def fromSocketAddress(socketAddress: InetSocketAddress): ConnectionManagerId = {
    new ConnectionManagerId(socketAddress.getHostName(), socketAddress.getPort())
  }
}
  
private[varys] class ConnectionManager(port: Int) extends Logging {
  ...
  val handleMessageExecutor = Executors.newFixedThreadPool(System.getProperty("varys.core.connection.handler.threads","20").toInt)
...
private[varys] object ConnectionManager {
